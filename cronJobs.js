...
const startCronJobs = () => {
    cron.schedule("0 0 * * *", async () => {
        try {
            ...
            // Find users to send reminder email
            const usersToSendMail = allUsers.filter((user) => {
                ...
                return formattedPlanEndDate.isAfter(moment(currentDate, "MM/DD/YYYY"), "day") ||
                    formattedPlanEndDate.isSame(moment(currentDate, "MM/DD/YYYY"), "day");

            });

            if (usersToSendMail.length > 0) {
                for (const user of usersToSendMail) {
            ...
            // Find users to update plan at the end of plan
            ...
            for (const user of usersToUpdate) {
                ...
                const terminationDate = moment()
                    .add(userPlan.planKey === "TRIAL" || plus.planKey
                        ? plus.duration.value
                        : userPlan.duration.value,
                        userPlan.planKey === "TRIAL" || plus.planKey
                        ? plus.duration.unit
                        : userPlan.duration.unit)
                    .format("MM/DD/YYYY");
