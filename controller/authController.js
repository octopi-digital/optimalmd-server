...
async function register(req, res) {
  try {
    ...
    if (!authToken) {
      addLog("Registration Error", null, `Authorization token is missing for getlyric during login attempt for user: ${userData?.firstName} ${userData?.lastName}, email: ${userData?.email}, phone: ${userData?.phone}.`);
      return res
        .status(401)
    ...
    if (role === "SalesPartner") {
      ...
      addLog("User Registration", newUser?._id, `New SalesPartner registered: ${newUser?.firstName} ${newUser?.lastName}, email: ${newUser?.email}, phone: ${newUser?.phone}.`);
      ...
      if (emailResponse.status !== 200) {
        addLog("Registration Error", newUser?._id, `Failed to send email for new SalesPartner registration: ${newUser?.firstName} ${newUser?.lastName}, email: ${newUser?.email}, phone: ${newUser?.phone}.`);
    ...
    if (paymentOption === "Card") {
      ...
      addLog("Registration Error", null, `Invalid payment details: Either card or bank account information must be provided for user: ${userData?.firstName} ${userData?.lastName}, email: ${userData?.email}, phone: ${userData?.phone}.`);
    ...
    addLog("Payment Record", newUser?._id, `Payment recorded for user: ${newUser?.firstName} ${newUser?.lastName}, email: ${newUser?.email}, phone: ${newUser?.phone}.`);
    ...
    addLog(
      "User Registration",
      newUser._id,
      `New user registered: ${newUser?.firstName} ${newUser?.lastName}, email: ${newUser?.email}, phone: ${newUser?.phone}.`
    );
...
async function updateUser(req, res) {
  try {
    ...
    if (!authToken) {
      addLog("Update User Error", userId, `Failed to login to Lyric: Authorization token is missing for getlyric. User ${user?.firstName} ${user?.lastName}, email: ${user?.email}, phone: ${user?.phone}.`);
      return res
        .status(401)
        .json({ error: "Authorization token missing for getlyric" });
    }
    ...
    if (!lyricsUserId) {
      ...
      if (!createMemberResponse.data.userid) {
        addLog("User Update Error", userId, `Failed to create a member in the Lyric system for user: ${user?.firstName} ${user?.lastName}, email: ${user?.email}, phone: ${user?.phone}.`);
        return res
          .status(500)
      ...
      if (!resp.data.success) {
        addLog("User Update Error", userId, `Failed to update member in Lyric system for user: ${user?.firstName} ${user?.lastName}, email: ${user?.email}, phone: ${user?.phone}.`);
        return res
          .status(500)
    ...
    if (!rxvaletID) {
      ...
      if (rxvaletResponse.data.StatusCode !== "1") {
        addLog("User Update Error", userId, `Failed to enroll member in RxValet system for user: ${user?.firstName} ${user?.lastName}, email: ${user?.email}, phone: ${user?.phone}.`);
      ...
      if (resp.data.StatusCode !== "1") {
        addLog("User Update Error", userId, `Failed to update member in RxValet system for user: ${user?.firstName} ${user?.lastName}, email: ${user?.email}, phone: ${user?.phone}.`);
        return res
          .status(500)
    ...
    addLog(
      "User Update",
      userId,
      `Updated user info for: ${updatedUser?.firstName} ${updatedUser?.lastName}, email: ${user?.email}, phone: ${user?.phone}.`
    );
    res.status(200).json({
...
async function updateUserPlan(req, res) {
  try {
    ...
    if (user.paymentOption === "Card") {
      ...
      addLog("Plan Update Error", userId, `Invalid payment details for user: ${user?.firstName} ${user?.lastName}, email: ${user?.email}, phone: ${user?.phone}.`);
      return res.status(400).json({
        success: false,
        error:
          "Invalid payment details. Provide either card or bank account information.",
    ...
    addLog("Payment Record", user?._id, `Payment recorded for user: ${user?.firstName} ${user?.lastName}, email: ${user?.email}, phone: ${user?.phone}.`);
    ...
    if (!authToken) {
      addLog("User Plan Update Error", userId, `Failed to log in to Lyric. Authorization token missing for getlyric. User: ${user?.firstName} ${user?.lastName}, email: ${user?.email}, phone: ${user?.phone}.`);
      return res
        .status(401)
        .json({ error: "Authorization token missing for getlyric" });
    }
    ...
    if (rxRespose.data.StatusCode !== "1") {
      addLog("User Plan Update Error", userId, `Failed to update user plan in the RxValet system for user: ${user?.firstName} ${user?.lastName}, email: ${user?.email}, phone: ${user?.phone}.`);
      return res.status(500).json({
    ...
    if (!response.data.success) {
      addLog("User Plan Update Error", userId, `Failed to update user in the Lyric system for user: ${user?.firstName} ${user?.lastName}, email: ${user?.email}, phone: ${user?.phone}.`);
      return res.status(500).json({
    ...
    addLog(
      "User Plan Update",
      userId,
      `Updated user plan for: ${updatedUser?.firstName} ${updatedUser?.lastName}, email: ${updatedUser?.email}, phone: ${updatedUser?.phone}.`
    );
    res.status(200).json({
...
async function updateUserImage(req, res) {
  try {
    ...
    if (!updatedUser) {
      addLog("Image Update Error", id, `User image update failed for user: ${user?.firstName} ${user?.lastName}, email: ${user?.email}, phone: ${user?.phone}.`);
...
async function deleteUser(req, res) {
  try {
    ...
    // Log the deletion
    addLog("Delete User", id, `Deleted user: ${user?.firstName} ${user?.lastName}, email: ${user?.email}, phone: ${user?.phone}.`);
...
async function updateUserStatus(req, res) {
  ...
  try {
    ...
    if (user.paymentOption === "Card") {
      ...
      addLog("User Update Error", currentUserId, `Invalid payment details for user: ${user?.firstName} ${user?.lastName}, email: ${user?.email}, phone: ${user?.phone}.`);
      return res.status(400).json({
        success: false,
        error:
          "Invalid payment details. Provide either card or bank account information.",
    ...
    if (!user.PrimaryMemberGUID && !user.lyricsUserId) {
      ...
      if (status === "Active") {
        try {
          ...
          if (paymentResponse.data?.transactionResponse?.transId === "0") {
            addLog("Payment Error", currentUserId, `Payment failed at the time of user status updating for user: ${user?.firstName} ${user?.lastName}, email: ${user?.email}, phone: ${user?.phone}.`);
            return res.status(500).json({
              success: false,
              error: "Payment Failed!",
            });
          ...
          addLog(
            "User Status Update",
            currentUserId,
            `Updated user status to "${status}" for: ${user?.firstName} ${user?.lastName}, email: ${user?.email}, phone: ${user?.phone}.`
          );
        } catch (err) {
      ...
      addLog("User Status Update", user?._id, `Status successfully updated to "${status}" for: ${user?.firstName} ${user?.lastName}, email: ${user?.email}, phone: ${user?.phone}.`);
      ...
      if (!cenSusauthToken) {
        addLog("User Update Error", currentUserId, `Failed to login to Lyric. Authorization token missing for GetLyric for user: ${user?.firstName} ${user?.lastName}, email: ${user?.email}, phone: ${user?.phone}.`);
        return res
          .status(401)
          .json({ error: "Authorization token missing for GetLyric." });
      }
      ...
      if (status === "Canceled") {
        ...
        try {
          ...
          addLog("User Status Update", user?._id, `Status successfully updated to "${status}" for user: ${user?.firstName} ${user?.lastName}, email: ${user?.email}, phone: ${user?.phone}.`);
        } catch (error) {
      ...
      try {
        ...
        addLog("User Update Error", currentUserId, `Failed to update status to "${status === "Active" ? "reactivate" : "terminate"}" user on GetLyric API for user: ${user?.firstName} ${user?.lastName}, email: ${user?.email}, phone: ${user?.phone}.`);
      ...
      try {
        ...
        addLog("User Update Error", currentUserId, `Failed to update status to "${status === "Active" ? "reactivate" : "terminate"}" user on RxValet API for user: ${user?.firstName} ${user?.lastName}, email: ${user?.email}, phone: ${user?.phone}.`);
      ...
      if (status === "Active") {
        ...
        console.log("lyrics data-: ", response.data);
        if (!response.data.success) {
          addLog("User Update Error", user?._id, `Failed to update user in Lyric system for user: ${user?.firstName} ${user?.lastName}, email: ${user?.email}, phone: ${user?.phone}.`);
          return res.status(500).json({
            error: "Failed to update user in Lyric system",
            data: response.data,
          });
        ...
        if (rxRespose.data.StatusCode !== "1") {
          addLog("User Update Error", user?._id, `Failed to update user plan in RxValet system for user: ${user?.firstName} ${user?.lastName}, email: ${user?.email}, phone: ${user?.phone}.`);
          return res.status(500).json({
            error: "Failed to update user plan in RxValet system",
            data: rxRespose.data,
          });
      ...
      addLog(
        "User Status Update",
        currentUserId,
        `Updated user status to "${status}" for user: ${user?.firstName} ${user?.lastName}, email: ${user?.email}, phone: ${user?.phone}.`
      );
...
async function manageUserRole(req, res) {
  ...
  try {
    ...
    addLog(
      "Update User Role",
      currentUserId,
      `Updated user role to "${role}" for user: ${user?.firstName} ${user?.lastName}, email: ${user?.email}, phone: ${user?.phone}.`
    );
